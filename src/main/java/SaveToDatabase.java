...
public class SaveToDatabase {
    ...
    isSubjectInKTCLASSTable(String subject, Connection conn) throws SQLException {
     ...
     * @param conn            for the connection to DB
     */
    private static void insertSubject(String englishSubject, String hebrewSubject, int activationOrder, int classIndex, String current_date, Connection conn) throws SQLException {
     ...
     * @param conn           for the connection to DB
     */
    private static void insertAttribute(String englishSubject, String englishField, String hebrewField, String type_name, int attributeIndex, String current_date, int keyType, int ioMode, Connection conn) throws SQLException {
            ...
            if (!isSubjectInKTCLASSTable(hebrewSubject, conn)) {
                if(isSubjectInKTCLASSTable(englishSubject, conn))
                {
                    ...
                    if (WITHLEVINSHTAINDISTANCE) {
                        ...
                        if (!HebrewSpellChecker.isSameWordInDBInKTCLASSTable(hebrewSubject, LEVINSHTAINDISTANCE, conn)) {
                            System.out.println("after levinshtain distance- not same");
                            insertSubject(englishSubject, hebrewSubject, ACTIVATION_ORDER, classIndex, CURRENT_DATE, conn);
                        }
                    } else {
                ...
                if (!isSubjectInKTATTRIBUTETable(englishSubject, hebrewField, conn, rs)) {
                    if(isSubjectInKTATTRIBUTETable(englishSubject, englishField, conn, rs))
                    {
                        ...
                        if (WITHLEVINSHTAINDISTANCE) {
                            ...
                            if (!HebrewSpellChecker.isSameWordInDBInKTATTRIBUTETable(hebrewSubject, hebrewField, LEVINSHTAINDISTANCE, conn)) {

                                insertAttribute(englishSubject, englishField, hebrewField, type_name,
                                        attributeIndex, CURRENT_DATE, keyType, IO_MODE, conn);
                            }
                        } else {
